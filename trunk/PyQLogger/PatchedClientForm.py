...
# HTMLParser is recent, so live without it if it's not available
try:
    import HTMLParser
except ImportError:
    ...
    def _request_data(self):
        ...
        if method == "GET":
            ...
            if "?" in self.action:
                ...
                fmt = "%s?%s"
            uri = fmt % (self.action, urlencode(self._pairs()))
            return uri, None, []
        elif method == "POST":
            if self.enctype == "application/x-www-form-urlencoded":
                ...
                data = StringIO()
                http_hdrs = []
		if hasattr(self,'referer'): http_hdrs += [("Referer",self.referer)]
