...
class updateCheckWorker(bgWorker):
	...
	def bg(self,*args):
		...
		while 42:			
			...
			max = LooseVersion(VERSION)
			req = urllib2.urlopen('http://savannah.nongnu.org/download/pyqlogger/')
			for line in req.readlines():
				m = re.search(r'"(PyQLogger-)(.*).tar.gz"',line)
				if m:
					newver = LooseVersion( m.group(2) )
					if newver > max:
						max = newver
			if max > cur and notified != max:
				notified = max
				self.OSD.info("New version %s is available at the site!"%(str(max)))
			sleep(60*30)
