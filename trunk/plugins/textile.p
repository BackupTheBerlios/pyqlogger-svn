...
class TextileTagPlugin(MenuPlugin):
    Name = "Textile tags"
    def abbr(self):
        editor = self.owner.sourceEditor
        ...
        editor.insertAt("%s(%s)" % (ab, text), line, index)
        
    def tagActivated(self, idx):
        t = self.Tags [ idx ]
        if t["Mode"] == "Custom":
            t["Value"]()
        elif t["Mode"] == "Surround":
            self.surroundWith(t["Value"])
        
    def surroundWith(self, tag):        
        editor = self.owner.sourceEditor
        text = unicode(editor.selectedText())
        editor.removeSelectedText()
        line, index = editor.getCursorPosition()
        editor.insertAt("%s%s%s" % (tag, text, tag), line, index)

    def getMenu( self, owner, parent):
        self.owner = owner
        self.Tags = [
        {"Name":"Make bold" , "Mode":"Surround" , "Value":"*" },
        {"Name":"Make italic" , "Mode":"Surround" , "Value":"__" },
        {"Name":"Code" , "Mode":"Surround" , "Value":"@" },
        {"Name":"Span" , "Mode":"Surround" , "Value":"%" },
        {"Name":"Abbriviate" , "Mode":"Custom" , "Value": self.abbr },
        ]
        tagmenu = QPopupMenu(parent)
        idx = 0
        for tag in self.Tags:
            tagmenu.insertItem(tag["Name"], idx)
            idx += 1
        parent.insertItem("Textile tags",tagmenu)
        parent.connect(tagmenu, SIGNAL('activated(int)'), self.tagActivated)
