...
class SpellCheck_Plugin(ToolBarManager.ToolbarPlugin):
	...
	def on_click(self):
		text = unicode(self.parent.sourceEditor.text())
		if not text or text == '':
			QMessageBox.information(None,	"Done!","Nothing to check!")
			return
		...
		res = s.exec_loop()
		if res:
			self.parent.sourceEditor.setText(s.text)
